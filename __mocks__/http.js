// Mock the HTTP module so that we can create HTTP servers and do HTTP requests
// without our own domain. This avoids TCP port numbers being allocated. This
// also avoids not closing servers in tests by accident.
...
class HttpServer extends EventEmitter {
  ...
  listen(port = 80, address = "localhost", backlog = 99, callback = undefined) {
...
function request(options, onResponse) {
  ...
  if (!server) {
    throw new Error(`connect ECONNREFUSED ${options.hostname}:${options.port}`);
  }
